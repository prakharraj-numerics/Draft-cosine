...
if [[ "$MODE" == "build" ]]; then
  ...
  rm -rf /tmp/highway /tmp/alpaka /tmp/taskflow /tmp/pthreadpool /tmp/pthreadpool-build /tmp/pthreadpool-install
  git clone --depth 1 --branch 1.4.0 https://github.com/google/highway.git /tmp/highway
  git clone --depth 1 https://github.com/alpaka-group/alpaka.git /tmp/alpaka
  git clone --depth 1 https://github.com/taskflow/taskflow.git /tmp/taskflow
  git clone --depth 1 https://github.com/Maratyszcza/pthreadpool.git /tmp/pthreadpool

  # Force pthreadpool's portable pthread/condvar backend on Apple instead of
  # its default GCD synchronization path, so this is a genuinely distinct test.
...
class AppleTaskflow2 {
    tf::Executor executor_{2};
...
public:
    AppleTaskflow2() {
        taskflow_.emplace(
            [this]{ cos53_eval_hwy(x_, y_, mid_); },
            [this]{ cos53_eval_hwy(x_ + mid_, y_ + mid_, n_ - mid_); }
        );
    }
    void run(const double *x,double *y,size_t n) {
        if(!n) return;
        if(n < 4) { cos53_eval_hwy(x,y,n); return; }
        x_=x; y_=y; n_=n; mid_=(n/2)&~(size_t)1;
        executor_.run(taskflow_).wait();
    }
};

struct AlpakaCos53SplitKernel {
    template<typename TAcc>
    ALPAKA_FN_ACC void operator()(TAcc const& acc, const double* x, double* y, size_t n, size_t mid) const
    {
        const size_t tid = static_cast<size_t>(alpaka::getIdx<alpaka::Grid, alpaka::Threads>(acc)[0]);
...
class AppleAlpaka2 {
    ...
    Dev dev_;
    Queue queue_;
    alpaka::WorkDivMembers<Dim,Idx> workdiv_;
    static Dev get_dev() { return alpaka::getDevByIdx(alpaka::Platform<Acc>{}, 0); }
public:
    AppleAlpaka2(): dev_(get_dev()), queue_(dev_), workdiv_(Vec{1},Vec{2},Vec{1}) {}
    void run(const double *x,double *y,size_t n) {
        if(!n) return;
        if(n < 4) { cos53_eval_hwy(x,y,n); return; }
        const size_t mid=(n/2)&~(size_t)1;
        alpaka::exec<Acc>(queue_, workdiv_, AlpakaCos53SplitKernel{}, x, y, n, mid);
        alpaka::wait(queue_);
    }
};
...
static int bench_one(const std::string& stack,size_t n)
{
    ...
    else if(stack=="alpaka") { AppleAlpaka2 r; wall=measure_runner(r,b,reps,&cpu); }
    else if(stack=="pthreadpool") { ApplePThreadPool2 r; wall=measure_runner(r,b,reps,&cpu); }
    else if(stack=="taskflow") { AppleTaskflow2 r; wall=measure_runner(r,b,reps,&cpu); }
    else return 3;
...
static int verify_one(const std::string& stack)
{
    ...
    if(stack=="alpaka") { AppleAlpaka2 r; return verify_runner(stack,r); }
    if(stack=="pthreadpool") { ApplePThreadPool2 r; return verify_runner(stack,r); }
    if(stack=="taskflow") { AppleTaskflow2 r; return verify_runner(stack,r); }
    return 3;
}
'''
...
PY
  ...
  clang++ -O3 -DNDEBUG -std=c++20 -mcpu=native -fno-fast-math -ffp-contract=off -fblocks \
    ...
    -I/tmp -I/tmp/highway -I/tmp/alpaka/include -I/tmp/taskflow -I/tmp/pthreadpool-install/include \
