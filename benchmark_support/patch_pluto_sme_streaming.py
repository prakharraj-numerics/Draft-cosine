...
# Namespace Apple SME ABI: ordinary C caller -> raw SMSTART wrapper -> __arm_streaming body.
...
# 64-byte streaming vector = 8 doubles. Scratch is outside the loop.
...
# SME_FA64=0: scalar-stage exact AoS coefficients. Crucially, leave NO SVE/SSVE
# value live across the scalar loop. Save jidx, delta, qbits using legal predicated
# stores; after staging recreate predicate and reload with legal predicated loads.
...
# Everything after staging uses the recreated predicate.
...
# Repair detection is fused into scalar staging.
