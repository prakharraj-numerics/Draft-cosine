...
# Real SME/ZA experiment. Keep PLUTO reduction, LUT, repair semantics, and Horner
# rounding order frozen; only execute the three polynomial FMAs through FP64 ZA.
s=s.replace('#include <arm_sve.h>\n', '#include <arm_sve.h>\n#include <arm_sme.h>\n', 1)

# Apple clang 17 on Namespace rejects __arm_new("za") in the locally-streaming
# declaration used here. Enter SM+ZA in the assembly wrapper and share ZA with
# this streaming body.
...
new_sig='''extern "C" size_t pluto_sme_stream(const double* __restrict x,double* __restrict y,size_t n,uint64_t* __restrict repairs) __arm_streaming __arm_inout("za");\nextern "C" size_t pluto_sme_stream(const double* __restrict x,double* __restrict y,size_t n,uint64_t* __restrict repairs) __arm_streaming __arm_inout("za") {'''
assert old_sig in s
s=s.replace(old_sig,new_sig,1)

old='''    size_t nr=0;\n    for(size_t i=0;i<n;i+=svcntd()){'''
new='''    size_t nr=0;\n    alignas(64) uint64_t ji_tmp[8];\n    alignas(64) uint64_t c0_bits[8];\n    alignas(64) uint64_t c1_bits[8];\n    alignas(64) uint64_t qbits_tmp[8];\n    alignas(64) double delta_tmp[8];\n    alignas(64) double za_rows[8][8];\n    alignas(64) double p_tmp[8];\n    const uint64_t* coeff_bits=reinterpret_cast<const uint64_t*>(opt_cos53_coeff_aos);\n    const svuint64_t lane_id=svindex_u64(0,1);\n    for(size_t i=0;i<n;i+=8){'''
assert old in s
...
# Exact original Horner:
#   c2 = c0*MH; c3 = c1*M6
#   p = fma(c3,d,c2)
#   p = fma(p,d,c1)
#   p = fma(p,d,c0)
# Each FMA is reproduced on the ZA diagonal. We seed the diagonal with the addend
# using predicated row writes, FMOPA the multiplicand vectors, then read back only
# the diagonal for the next Horner stage. This preserves the original operation
# ordering instead of algebraically regrouping the polynomial.
...
assert old in s
new='''        svfloat64_t c2=svmul_n_f64_x(pg2,c0,MH);\n        svfloat64_t c3=svmul_n_f64_x(pg2,c1,M6);\n\n        svzero_za();\n        for(uint64_t r=0;r<lanes;r++){ svbool_t diag=svcmpeq_n_u64(pg2,lane_id,r); svwrite_hor_za64_f64_m(0,(uint32_t)r,diag,c2); }\n        svmopa_za64_f64_m(0,pg2,pg2,c3,delta);\n        for(uint64_t r=0;r<lanes;r++) svst1_hor_za64(0,(uint32_t)r,pg2,za_rows[r]);\n        for(uint64_t l=0;l<lanes;l++) p_tmp[l]=za_rows[l][l];\n        svfloat64_t p=svld1_f64(pg2,p_tmp);\n\n        svzero_za();\n        for(uint64_t r=0;r<lanes;r++){ svbool_t diag=svcmpeq_n_u64(pg2,lane_id,r); svwrite_hor_za64_f64_m(0,(uint32_t)r,diag,c1); }\n        svmopa_za64_f64_m(0,pg2,pg2,p,delta);\n        for(uint64_t r=0;r<lanes;r++) svst1_hor_za64(0,(uint32_t)r,pg2,za_rows[r]);\n        for(uint64_t l=0;l<lanes;l++) p_tmp[l]=za_rows[l][l];\n        p=svld1_f64(pg2,p_tmp);\n\n        svzero_za();\n        for(uint64_t r=0;r<lanes;r++){ svbool_t diag=svcmpeq_n_u64(pg2,lane_id,r); svwrite_hor_za64_f64_m(0,(uint32_t)r,diag,c0); }\n        svmopa_za64_f64_m(0,pg2,pg2,p,delta);\n        for(uint64_t r=0;r<lanes;r++) svst1_hor_za64(0,(uint32_t)r,pg2,za_rows[r]);\n        for(uint64_t l=0;l<lanes;l++) p_tmp[l]=za_rows[l][l];\n        p=svld1_f64(pg2,p_tmp);\n\n        svuint64_t parity=svand_u64_x(pg2,qbits,oneu); svuint64_t outsign=svlsl_n_u64_x(pg2,parity,63); p=svreinterpret_f64_u64(sveor_u64_x(pg2,svreinterpret_u64_f64(p),outsign)); svst1_f64(pg2,y+i,p);\n'''
s=s.replace(old,new,1)
...
'''// SME/ZA integration: frozen PLUTO reduction/LUT plus original Horner rounding,\n// with all three polynomial FMAs executed on the M4 FP64 ZA matrix engine.''')
