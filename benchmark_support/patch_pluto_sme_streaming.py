...
# Apple clang 17 on the Namespace M4 rejects __arm_new("za") here.  Use an
# explicit ordinary-C assembly entry that owns SM+ZA, and make the body a
# streaming ZA-sharing function.  This mirrors the ABI pattern already proven on
# this runner, now with ZA enabled as well as streaming mode.
...
# only the diagonal, which corresponds to independent input lanes. The existing
# <=2 ULP MPFR gate decides whether this regrouping is acceptable.
