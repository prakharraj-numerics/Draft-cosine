...
# Known-good Namespace Apple SME ABI:
# ordinary C caller -> hand-written SMSTART wrapper -> __arm_streaming body.
# Keep the wrapper as the sole normal-path PSTATE.SM transition.
...
# Namespace M4/M5 exposes a 64-byte streaming vector length: 8 doubles.
# Keep only one 8-lane scratch set, aligned to one streaming vector, outside the loop.
...
# SME_FA64=0 on these hosts, so no indexed Streaming-SVE gathers.
# Preserve exact K1280 coefficients via one fused scalar staging/repair pass.
...
# Ordinary ABI entry point; raw assembly wrapper enters/leaves streaming mode once.
