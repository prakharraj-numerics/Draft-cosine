...
// B: full reduction/index/staging + Horner, but coefficient-table memory removed.
__arm_locally_streaming static void pluto_diag_B(const double*x,double*y,size_t n){
 const svuint64_t sm=svdup_u64(UINT64_C(0x8000000000000000)),one=svdup_u64(1),jm=svdup_u64((UINT64_C(1)<<52)-1);alignas(64)uint64_t qtmp[8];
...
// C: polynomial/sign only; delta/c0/c1/q parity precomputed outside timing.
__arm_locally_streaming static void pluto_diag_C(const double*x,double*y,size_t n){auto&z=diag_prep(x,n);const svuint64_t one=svdup_u64(1);for(size_t i=0;i<n;i+=8){svbool_t pg=svwhilelt_b64((uint64_t)i,(uint64_t)n);svfloat64_t de=svld1_f64(pg,z.delta.data()+i),c0=svld1_f64(pg,z.c0.data()+i),c1=svld1_f64(pg,z.c1.data()+i);svfloat64_t c2=svmul_n_f64_x(pg,c0,MH),c3=svmul_n_f64_x(pg,c1,M6),v=svmla_f64_x(pg,c2,c3,de);v=svmla_f64_x(pg,c1,v,de);v=svmla_f64_x(pg,c0,v,de);svuint64_t qb=svld1_u64(pg,z.qbits.data()+i),os=svlsl_n_u64_x(pg,svand_u64_x(pg,qb,one),63);v=svreinterpret_f64_u64(sveor_u64_x(pg,svreinterpret_u64_f64(v),os));svst1_f64(pg,y+i,v);}}

// D: j/delta/q parity precomputed; real random LUT fetch + Horner timed.
__arm_locally_streaming static void pluto_diag_D(const double*x,double*y,size_t n){auto&z=diag_prep(x,n);const svuint64_t one=svdup_u64(1);alignas(64)double a0[8],a1[8];for(size_t i=0;i<n;i+=8){size_t lanes=std::min<size_t>(8,n-i);for(size_t l=0;l<lanes;l++){uint64_t j=z.j[i+l];if(j>=2009)j=0;a0[l]=opt_cos53_coeff_aos[2*j];a1[l]=opt_cos53_coeff_aos[2*j+1];}svbool_t pg=svwhilelt_b64((uint64_t)i,(uint64_t)n);svfloat64_t de=svld1_f64(pg,z.delta.data()+i),c0=svld1_f64(pg,a0),c1=svld1_f64(pg,a1);svfloat64_t c2=svmul_n_f64_x(pg,c0,MH),c3=svmul_n_f64_x(pg,c1,M6),v=svmla_f64_x(pg,c2,c3,de);v=svmla_f64_x(pg,c1,v,de);v=svmla_f64_x(pg,c0,v,de);svuint64_t qb=svld1_u64(pg,z.qbits.data()+i),os=svlsl_n_u64_x(pg,svand_u64_x(pg,qb,one),63);v=svreinterpret_f64_u64(sveor_u64_x(pg,svreinterpret_u64_f64(v),os));svst1_f64(pg,y+i,v);}}

// E: exact full path, but stage 64/128 elements before the LUT/poly pass.
template<size_t BS> __arm_locally_streaming static void pluto_diag_E(const double*x,double*y,size_t n){
...
static void pluto_E64(const double*x,double*y,size_t n){pluto_diag_E<64>(x,y,n);} static void pluto_E128(const double*x,double*y,size_t n){pluto_diag_E<128>(x,y,n);}
struct BRunner{void run(const double*x,double*y,size_t n){pluto_diag_B(x,y,n);}};struct CRunner{void run(const double*x,double*y,size_t n){(void)diag_prep(x,n);pluto_diag_C(x,y,n);}};struct DRunner{void run(const double*x,double*y,size_t n){(void)diag_prep(x,n);pluto_diag_D(x,y,n);}};struct E64Runner{void run(const double*x,double*y,size_t n){pluto_E64(x,y,n);}};struct E128Runner{void run(const double*x,double*y,size_t n){pluto_E128(x,y,n);}};
'''
...
# Extend MPFR validation to exact E candidates; B/C/D are diagnostic decompositions.
old='score("neon","9600",x,pluto_neon);score("sme","9600",x,pluto_sme);'
new='score("neon","9600",x,pluto_neon);score("sme","9600",x,pluto_sme);score("E64","9600",x,pluto_E64);score("E128","9600",x,pluto_E128);'
assert old in s;s=s.replace(old,new,1)
old='score("sme","1m_stress",x,pluto_sme);return 0;'
new='score("sme","1m_stress",x,pluto_sme);score("E64","1m_stress",x,pluto_E64);score("E128","1m_stress",x,pluto_E128);return 0;'
assert old in s;s=s.replace(old,new,1)
p.write_text(s)
