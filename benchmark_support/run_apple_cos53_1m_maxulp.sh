...
BASE=e114c2a749b4a7287573f577582eff2d4c9f3681
...
git show "$BASE":apple_cos53_production_routing.hpp > /tmp/apple_cos53_production_routing.hpp
...
#include <Halide.h>
int main() { Halide::compile_standalone_runtime("/tmp/halide_runtime.a", Halide::get_host_target()); return 0; }
CPP
clang++ -O2 -std=c++20 -I"$HP/include" /tmp/genrt.cpp -L"$HP/lib" -lHalide -o /tmp/genrt
/tmp/genrt

cp /tmp/base.cpp /tmp/validate.cpp
python3 - <<'PY'
from pathlib import Path
p=Path('/tmp/validate.cpp'); s=p.read_text()
...
s=s.replace(marker,'#include "apple_cos53_production_routing.hpp"\n\n'+marker,1)
a=s.index('static int validate_mode()')
b=s.index('\n#endif', a)
new='''static int validate_mode()\n{\n    const int N=1000000;\n    std::vector<double> x(N),o(N);\n    const double lo[3]={0.0,1.0,1000.0},hi[3]={1.0,500.0,10000.0};\n    for(int i=0;i<N;i++) {\n        int c=i%6,b=c/2;\n        double u=unit52(mix64(UINT64_C(2026090599)+(uint64_t)i*UINT64_C(0x9e3779b97f4a7c15)));\n        double v=std::fma(hi[b]-lo[b],u,lo[b]);\n        x[i]=(c&1)?-v:v;\n    }\n    AppleTwoCoreHighway tc;\n    apple_cos53_production::run(tc,x.data(),o.data(),N);\n    mpfr_t z,r; mpfr_init2(z,256); mpfr_init2(r,256);\n    uint64_t maxulp=0; int exact=0,le1=0,le2=0,worst=-1,bad=0;\n    double worst_x=0.0,worst_out=0.0,worst_ref=0.0;\n    for(int i=0;i<N;i++) {\n        mpfr_set_d(z,x[i],MPFR_RNDN); mpfr_cos(r,z,MPFR_RNDN);\n        double ref=mpfr_get_d(r,MPFR_RNDN); uint64_t u=ulpd(o[i],ref);\n        if(u==0) exact++; if(u<=1) le1++; if(u<=2) le2++;\n        if(u>2){bad++; std::printf(\"OUTLIER i=%d x=%.17g out=%.17g ref=%.17g ulp=%llu abs_err=%.17g\\n\",i,x[i],o[i],ref,(unsigned long long)u,std::fabs(o[i]-ref));}\n        if(u>maxulp){maxulp=u;worst=i;worst_x=x[i];worst_out=o[i];worst_ref=ref;}\n    }\n    mpfr_clear(r); mpfr_clear(z);\n    std::printf(\"APPLE_COS53_PRODUCTION_1M_VALIDATE cases=%d exact=%d le1=%d le2=%d bad=%d maxulp=%llu worst_i=%d worst_x=%.17g worst_out=%.17g worst_ref=%.17g reference=MPFR256\\n\",N,exact,le1,le2,bad,(unsigned long long)maxulp,worst,worst_x,worst_out,worst_ref);\n    return 0;\n}\n'''
s=s[:a]+new+s[b:]
p.write_text(s)
PY
MP="$(brew --prefix mpfr)"; GP="$(brew --prefix gmp)"
clang++ -O3 -DNDEBUG -std=c++20 -mcpu=native -fno-fast-math -ffp-contract=off -fblocks -DAPPLE_COS53_VALIDATE_MPFR -I/tmp -I/tmp/highway -I"$HP/include" -I"$MP/include" -I"$GP/include" /tmp/validate.cpp /tmp/halide_runtime.a -L"$MP/lib" -L"$GP/lib" -lmpfr -lgmp -framework Accelerate -pthread -ldl -o /tmp/validate
/tmp/validate validate | tee /tmp/result.txt
