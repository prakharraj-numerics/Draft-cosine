...
/* COS53 FROZEN custom permanent 2-core scheduler.
   ...
   Frozen from the exact Intel Xeon 6973P-C three-way benchmark:
     workflow run 33562041646, shard 41

   Validated behavior on the requested grid:
     - custom2 output was bit-identical to the current COS53 evaluator
       across all 72 tested size/sign/range cells
     - custom2 beat current COS53 in every tested cell at n >= 5000
     - tested n >= 5000 points: 5000, 10000, 30000, 50000, 100000,
...
class Cosine53CustomPermanent2Core5000Frozen {
public:
    using fn_t = void (*)(double *, const double *, size_t);

    explicit Cosine53CustomPermanent2Core5000Frozen(fn_t fn)
    ...
    ~Cosine53CustomPermanent2Core5000Frozen() {
        stop_.store(true, std::memory_order_relaxed);
        generation_.fetch_add(1, std::memory_order_release);
        if (helper_.joinable()) helper_.join();
    }

    Cosine53CustomPermanent2Core5000Frozen(const Cosine53CustomPermanent2Core5000Frozen&) = delete;
    Cosine53CustomPermanent2Core5000Frozen& operator=(const Cosine53CustomPermanent2Core5000Frozen&) = delete;
