...
static int run_native(const std::string &which) {
    ...
    Cosine53BatchLazyCandidate *lazy = nullptr;
    ...
    for (size_t n : sizes) {
        Buffers b;
        if (!alloc_buffers(b, n)) { delete lazy; delete prod; return 10; }
        ...
        auto once = [&]() {
            for (size_t j = 0; j < cases; ++j) {
                ...
                else vmdCos((MKL_INT)n, b.x[j], b.y[j], VML_HA);
            }
        };
        ...
        if (which != "intel") {
            once();
            for (size_t j = 0; j < cases; ++j)
                vmdCos((MKL_INT)n, b.x[j], b.ref[j], VML_HA);
            for (size_t j = 0; j < cases; ++j)
                for (size_t i = 0; i < n; ++i)
                    maxulp = std::max(maxulp, ulp_distance(b.y[j][i], b.ref[j][i]));
        }
        ...
        const int helper = lazy ? (lazy->helper_started() ? 1 : 0) : (prod ? 1 : 0);
        std::printf("LAZY_NATIVE engine=%s stack=%s n=%zu cases=%zu reps=%zu wall_ns_el=%.9f cpu_ns_el=%.9f effective_cores=%.6f maxrss_kib=%ld helper_started=%d maxulp_vs_intel=%llu\n",
                    COS53_ENGINE_WIDE ? "wide" : "unit", which.c_str(), n, cases, reps,
                    mw, mc, mw > 0.0 ? mc/mw : 0.0, ru.ru_maxrss, helper,
                    (unsigned long long)maxulp);
    }

    delete lazy;
    delete prod;
    std::printf("LAZY_NATIVE_DONE engine=%s stack=%s sink=%.17g\n",
...
int main(int argc, char **argv) {
    ...
    int rc = 2;
    if (which == "prod" || which == "lazy" || which == "intel") rc = run_native(which);
