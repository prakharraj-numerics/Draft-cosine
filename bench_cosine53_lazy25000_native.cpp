...
#include "cosine53_batch_lazy_25000_candidate.hpp"
...
static const size_t kSizes[] = {100,700,3500,4500,5000,8000,15000,20000,25000,30000,50000,1000000,2000000};
...
struct Buffers{std::vector<double*>x,y,ref;~Buffers(){for(double*p:x)free(p);for(double*p:y)free(p);for(double*p:ref)free(p);}};
static bool alloc_buffers(Buffers&b,size_t n){int c0=COS53_ENGINE_WIDE?2:0,c1=COS53_ENGINE_WIDE?6:2,nc=c1-c0;b.x.resize(nc);b.y.resize(nc);b.ref.resize(nc);for(int j=0;j<nc;++j){b.x[j]=(double*)al64(n*sizeof(double));b.y[j]=(double*)al64(n*sizeof(double));b.ref[j]=(double*)al64(n*sizeof(double));if(!b.x[j]||!b.y[j]||!b.ref[j])return false;fill(b.x[j],n,c0+j);}return true;}
...
static int run_native(const std::string&which){
    Cosine53BatchProductionFrozen*prod=nullptr;Cosine53BatchLazy25000Candidate*lazy=nullptr;
    if(which=="prod")prod=new Cosine53BatchProductionFrozen(cos53_engine_eval);else if(which=="lazy25000")lazy=new Cosine53BatchLazy25000Candidate(cos53_engine_eval);
    for(size_t n:kSizes){
        Buffers b;if(!alloc_buffers(b,n)){delete lazy;delete prod;return 10;}size_t cases=b.x.size(),reps=reps_for(n,cases);
        auto once=[&](){for(size_t j=0;j<cases;++j){if(which=="prod")prod->run(b.y[j],b.x[j],n);else if(which=="lazy25000")lazy->run(b.y[j],b.x[j],n);else vmdCos((MKL_INT)n,b.x[j],b.y[j],VML_HA);}};
        ...
        rusage ru{};getrusage(RUSAGE_SELF,&ru);double mw=median5(wall),mc=median5(cpu);int helper=lazy?(lazy->helper_started()?1:0):(prod?(prod->helper_started()?1:0):0);
        printf("L25000_NATIVE engine=%s stack=%s n=%zu cases=%zu reps=%zu wall_ns_el=%.9f cpu_ns_el=%.9f effective_cores=%.6f maxrss_kib=%ld helper_started=%d maxulp_vs_intel=%llu\n",COS53_ENGINE_WIDE?"wide":"unit",which.c_str(),n,cases,reps,mw,mc,mw?mc/mw:0.0,ru.ru_maxrss,helper,(unsigned long long)maxulp);
    }
    delete lazy;delete prod;printf("L25000_NATIVE_DONE engine=%s stack=%s sink=%.17g\n",COS53_ENGINE_WIDE?"wide":"unit",which.c_str(),(double)g_sink);return 0;
}

int main(int argc,char**argv){if(argc!=2)return 2;pin_current(0);mkl_set_num_threads_local(1);if(!cos53_engine_init())return 3;std::string which=argv[1];int rc=2;if(which=="prod"||which=="lazy25000"||which=="intel")rc=run_native(which);cos53_engine_cleanup();return rc;}
